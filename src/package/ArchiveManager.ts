...
export class ArchiveManager {
  ...
  private static createArchiver(format: string, compressLevel: number = 9): archiver.Archiver {
    switch (format) {
      case 'zip':
        return archiver('zip', { zlib: { level: compressLevel } })
      case 'tar.gz':
        return archiver('tar', { gzip: true, gzipOptions: { level: compressLevel } })
      case 'tar.xz':
        return archiver('tar', {})
      case 'tar.bz2':
        return archiver('tar', {})
  ...
  private static addPackageMetadata(archive: archiver.Archiver, packageInfo: PackageInfo): void {
    const metadata = {
  ...
  private static async addSourceCodeToArchive(archive: archiver.Archiver): Promise<void> {
    try {
  ...
  private static async findGoFiles(dir: string): Promise<string[]> {
    try {
      const { stdout } = await execa('find', [dir, '-name', '*.go', '-type', 'f'])
