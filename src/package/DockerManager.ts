...
export class DockerManager {
  ...
  static async pushImage(imageName: string, registry?: string, tag?: string): Promise<void> {
    const fullImageName = registry ? `${registry}/${imageName}` : imageName
    const targetTag = tag || 'latest'

    try {
      // Tag image
      await execa('docker', ['tag', imageName, `${fullImageName}:${targetTag}`])
  ...
  static async generateDockerfile(binaryPath: string, options: DockerOptions): Promise<string> {
...
`
  ...
  private static generateMultiStageDockerfile(binaryName: string, options: DockerOptions): string {
    return `# Build stage
...
`
  ...
  static async saveImage(imageName: string, outputDir: string): Promise<string> {
