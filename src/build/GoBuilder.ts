...
import os from 'os'
import { type BuildConfig, type BuildTarget, type BuildResult } from './../types.js'
...
export class GoBuilder {
  ...
  private async buildTargetsParallel(config: BuildConfig): Promise<void> {
    ...
    for (const batch of batches) {
      await Promise.allSettled(batch.map(target => this.buildTargetWithCache(target, config)))
    }
  }
  private async buildTargetWithCache(target: BuildTarget, config: BuildConfig): Promise<void> {
    try {
      const cacheKey = await this.buildCache.getCacheKey(target, config)
      const cacheHit = await this.buildCache.restoreFromCache(cacheKey, target.output)
      if (cacheHit) {
  ...
  private async executeGoBuild(target: BuildTarget, config: BuildConfig): Promise<void> {
  ...
  private async generateChecksums(config: BuildConfig): Promise<void> {
    this.checksumFiles = await ChecksumUtil.writeChecksumFiles(this.builtBinaries, config.outputDir)
