...
export class CLI {
  ...
  private setupCLI(): void {
    this.program
      .name('mkgo')
      .description('🚀 Modern Go Build Orchestrator - Fast, reliable cross-platform Go builds')
      .version('2.0.0', '-v, --version', 'Display version information')
      .helpOption('-h, --help', 'Display help information')
    const buildCommand = new Command('build')
      .description('Build Go project with advanced features')
      .option('--target <platforms...>', 'Target platforms (e.g., linux/amd64,darwin/arm64)')
      .option('--os <os...>', 'Target operating systems (linux, darwin, windows)')
      ...
      .option('--metrics', 'Generate build metrics and performance report', false)
      .option('--benchmark', 'Compare build performance with previous run', false)
    ...
    const initCommand = new Command('init')
      ...
      .option('--template <type>', 'Project template (basic, api, cli, microservice)', 'basic')
    ...
    const listCommand = new Command('list')
      .description('List available platforms, architectures, and configurations')
    ...
    const metricsCommand = new Command('metrics')
      ...
      .option('--compare <commit>', 'Compare with previous git commit', 'HEAD~1')
    ...
    const packageCommand = new Command('package')
      ...
      .option('--format <format>', 'Package format (zip, tar.gz, dmg, deb, rpm, msi, exe)', 'zip')
    ...
    const installerCommand = new Command('installer')
      ...
      .option('--type <type>', 'Installer type (msi, exe, pkg, deb, rpm)', 'msi')
  ...
  private async createBuildConfig(options: CLIOptions & any): Promise<BuildConfig> {
  ...
  private determineTargets(options: CLIOptions & any): BuildTarget[] {
    ...
    if (options.all) {
      ...
      platforms = Array.isArray(options.target) ? options.target : [options.target]
...
${pc.gray('Run "mkgo metrics --report" for detailed performance analysis')}
  ...
  private async clean(options: any): Promise<void> {
    ...
    try {
      const directories = ['dist', 'build', 'bin', '.mkgo-cache', '.build-metrics']
...
`,
      ...
      Logger.success(`Project initialized successfully with ${template} template!`)
  ...
  private async info(options: any): Promise<void> {
    try {
      ...
      const showAll =
        options.all || (!options.system && !options.go && !options.project && !options.cache)
...
`.trim()
      ...
      if (options.go || showAll) {
        ...
        try {
          ...
          const goEnv = await execa('go', ['env', 'GOPATH', 'GOROOT', 'GOOS', 'GOARCH'])
...
`.trim()
  ...
  private async list(options: any): Promise<void> {
    try {
      const showAll =
        options.all ||
        (!options.platforms && !options.archs && !options.templates && !options.presets)
      if (options.platforms || showAll) {
        console.log(pc.blue('\n🌍 Supported Platforms:'))
        SUPPORTED_PLATFORMS.forEach(platform => {
          const [os, arch] = platform.split('/')
          const osName =
            os === 'darwin' ? 'macOS' : os === 'linux' ? 'Linux' : os === 'windows' ? 'Windows' : os
...
`.trim()
  ...
  private async generateBuildMetrics(config: BuildConfig, result: BuildResult): Promise<void> {
  ...
  private async generateMetricsReport(): Promise<void> {
    ...
    const avgCacheEfficiency =
      metrics.reduce((sum, m) => sum + (m.cacheEfficiency || 0), 0) / metrics.length
    console.log(
      `
...
`.trim()
  ...
  private async listPlatforms(): Promise<void> {
    ...
    SUPPORTED_PLATFORMS.forEach(platform => {
      ...
      const osName =
        os === 'darwin' ? 'macOS' : os === 'linux' ? 'Linux' : os === 'windows' ? 'Windows' : os
      console.log(`  ${pc.cyan(platform.padEnd(15))} ${osName} ${pc.gray(arch)}`)
    })
